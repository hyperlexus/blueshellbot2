...
from Config.Exceptions import BadCommandUsage, InvalidInput, NumberRequired, UnknownError, BlueshellError
...
class JumpMusicHandler(AbstractHandler):
    """Move a song from a specific position and play it now"""
    ...
    async def run(self, musicPos: str) -> HandlerResponse:
        ...
        if acquired:
            ...
            try:
                ...
                command = BCommands(BCommandsType.SKIP, None)
    ...
    def __validateInput(self, position: str) -> Union[BlueshellError, None]:
        try:
