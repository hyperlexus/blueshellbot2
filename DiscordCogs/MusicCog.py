...
class MusicCog(Cog):
    ...
    async def play(self, ctx: Context, *args) -> None:
        try:
            ...
            if response is not None:
                cog_responder1 = EmbedCommandResponse(response, MessagesCategory.PLAYER)
                cog_responder2 = EmoteCommandResponse(response, MessagesCategory.PLAYER)
                await cog_responder1.run()
                await cog_responder2.run()
    ...
    async def queue(self, ctx: Context, *args) -> None:
        try:
            ...
            if page_number == "":
                ...
                page_number -= 1  # Change index 1 to 0
                response = await controller.run(page_number)

            cog_responder = EmbedCommandResponse(response, MessagesCategory.QUEUE)
            await cog_responder.run()
        except ValueError as e:
            # Draft a Handler Response to pass to cog_responder
            error = InvalidInput()
            embed = self.__embeds.INVALID_ARGUMENTS()
            response = HandlerResponse(ctx, embed, error)

            cog_responder = EmbedCommandResponse(response, MessagesCategory.QUEUE)
            await cog_responder.run(deleteLast=False)
        except Exception as e:
            print(f'[ERROR IN COG] -> {e}')
    ...
    async def pause(self, ctx: Context) -> None:
        try:
            ...
            response = await controller.run()
            cog_responder1 = EmoteCommandResponse(response, MessagesCategory.PLAYER)
            cog_responder2 = EmbedCommandResponse(response, MessagesCategory.PLAYER)
            await cog_responder1.run()
            await cog_responder2.run()
        except Exception as e:
            print(f'[ERROR IN COG] -> {e}')
    ...
    @command(name='prev', help=helper.HELP_PREV, description=helper.HELP_PREV_LONG, aliases=['return', 'previous', 'back', 'last'])
    @is_not_banned()
    ...
    async def history(self, ctx: Context) -> None:
        try:
            ...
            response = await controller.run()
            cog_responder1 = EmbedCommandResponse(response, MessagesCategory.HISTORY)
            cog_responder2 = EmoteCommandResponse(response, MessagesCategory.HISTORY)
            await cog_responder1.run()
            await cog_responder2.run()
        except Exception as e:
            print(f'[ERROR IN COG] -> {e}')
    ...
    async def loop(self, ctx: Context, args='') -> None:
        try:
            ...
            cog_responder1 = EmoteCommandResponse(response, MessagesCategory.LOOP)
            cog_responder2 = EmbedCommandResponse(response, MessagesCategory.LOOP)
            await cog_responder1.run()
            await cog_responder2.run()
        except Exception as e:
            print(f'[ERROR IN COG] -> {e}')
    ...
    async def now_playing(self, ctx: Context) -> None:
        try:
            ...
            response = await controller.run()
            cog_responder1 = EmbedCommandResponse(response, MessagesCategory.NOW_PLAYING)
            cog_responder2 = EmoteCommandResponse(response, MessagesCategory.NOW_PLAYING)
            await cog_responder1.run()
            await cog_responder2.run()
        except Exception as e:
            print(f'[ERROR IN COG] -> {e}')
    ...
    async def move(self, ctx: Context, pos1, pos2='1') -> None:
        try:
            ...
            cog_responder1 = EmbedCommandResponse(response, MessagesCategory.MANAGING_QUEUE)
            cog_responder2 = EmoteCommandResponse(response, MessagesCategory.MANAGING_QUEUE)
            await cog_responder1.run()
            await cog_responder2.run()
        except Exception as e:
            print(f'[ERROR IN COG] -> {e}')
