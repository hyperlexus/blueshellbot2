...
class ProcessPlayerManager(Singleton, AbstractPlayersManager):
    ...
    def resetPlayer(self, guild: Guild, context: Context) -> None:
        """Restart a running process, if it was playing make it play again"""
        if guild.id not in self.__playersProcess.keys():
            return None
    ...
    def __listenToCommands(self, queue: Queue, guild: Guild) -> None:
        ...
        while True:
            ...
            try:
                command: BCommands = queue.get(timeout=5)
                commandType = command.getType()
                args = command.getArgs()

                print(f'Command {commandType} from server "{guild.name}"')
    ...
    def __putCommandInQueue(self, queue: Queue, command: BCommands) -> None:
        try:
            queue.put(command)
        except Exception as e:
