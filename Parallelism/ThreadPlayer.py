...
class ThreadPlayer(Thread):
    ...
    def __init__(self, bot: BlueshellBot, guild: Guild, name: str, voiceChannel: VoiceChannel, playlist: Playlist, lock: Lock, guildID: int, voiceID: int, callbackToSendCommand: Callable, exitCB: Callable) -> None:
    ...
    async def receiveCommand(self, command: BCommands) -> None:
        try:
            self.__playerLock.acquire()
